...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  const current = unitOfWork.alternate;// react render 过程中会有2棵树，一个是当前的dom结构树，current,一棵是渲染过程中构建的树，workingProgressTree上节点的alternate 属性指向 current Tree 上对应的节点
  ...
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode) {//enableProfilerTimer 且 unitOfWork.mode 是 ProfileMode
    ...
    next = beginWork(current, unitOfWork, subtreeRenderLanes); // beginWork 将结果赋值给next,后面我们会知道这是一个深度优先的递归，会从根节点一路向下找子节点。当子节点为null,即没有子节点时，即next == null ,这时进行completeWork，通过completeWork,后会把兄弟节点当做设为workProgress,然后return，代码继续回到performUnitOfWork (注意这时workProgress 已经变成了上一个节点的sibling 兄弟几点，然后会继续在这个兄弟节点上 begainWork，当到了最后一个sibling==null时，completeWork把 next设置成父节点，在父节点上执行completeWork, 然后会进入到父节点的sibling, beginWork -> completeWork,一直到根节点  )
  }

  resetCurrentDebugFiberInDEV();
  unitOfWork.memoizedProps = unitOfWork.pendingProps;
  if (next === null) {// 没有子节点了，执行completeUnitOfWork 把 兄弟节点作为下个workProgress
